...
int valor_lexico;
char *code;
int num_linha = 1; // O contador de linhas começa em 1, pois não tem linha zero
...
void build_parse_table()
{
    ...
    // If -> IF ( Expr ) Stmt IfTail
    parse_table[NT_IF][IF] = { IF, ABRE_PARENTESES, NT_EXPR, FECHA_PARENTESES, NT_STMT, NT_IF_TAIL };
    ...
    parse_table[NT_WHILE][WHILE] = { WHILE, ABRE_PARENTESES, NT_EXPR, FECHA_PARENTESES, ABRE_CHAVES, NT_STMT_LIST, FECHA_CHAVES };
    ...
    // epsilon cases
    std::vector<int> addtail_eps = { RELOP, FECHA_PARENTESES, PONTO_VIRGULA, EOF, FIM, FECHA_CHAVES };
    ...
    // epsilon cases
    std::vector<int> termtail_eps = { ADD, SUB, RELOP, FECHA_PARENTESES, PONTO_VIRGULA, EOF, FIM, FECHA_CHAVES };
    ...
    while(!st.empty()){
        ...
        if(!is_nonterm(top)){
            ...
            if(top == cur.nome_token){
                ...
                printf("\nErro sintático na linha %d: token esperado '%s', encontrado '%s' (pos %d)\n",
                    num_linha, token_name(top).c_str(), token_name(cur.nome_token).c_str(), cont_sim_lido);
                exit(EXIT_FAILURE);
            }
        } else {
            ...
            if(itProd == row.end()){
                ...
                printf("\nErro sintático na linha %d: não há produção para não-terminal %s com token %s (pos %d)\n",
                    num_linha, nonterm_to_string(top).c_str(), token_name(cur.nome_token).c_str(), cont_sim_lido);
                exit(EXIT_FAILURE);
            }
